...
> [!WARNING]
> **版本升级**：跨大版本的升级极有可能导致数据库不兼容，并导致阅读进度数据丢失，请谨慎升级。
...
- `APP_PASSWORD`：**[可选]** 应用访问密码。
  - 如果设置：启动时需输入密码，且服务器重启后需重新登录（强制下线）。
  - 如果未设置：应用的 API 和前端将完全公开（默认）。
...
**Glean** 强调"服务端即本体"。你的所有书籍和阅读记录都留在服务器上，前端仅作为一个轻量级的窗口。无论是在电脑、手机还是平板上，阅读体验都是连贯且无缝的。
