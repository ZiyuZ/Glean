...
- 🔍 **搜索过滤**：支持书架书籍的关键字搜索，以及按阅读状态（进行中/已读/收藏）筛选。
- ⚡️ **智能解析**：优化的正则表达式解析引擎，精准提取章节目录，解决复杂中文语境下的误判。
- 📚 **云端书架**：多端同步的阅读进度，自动保存最后一次阅读位置，点开即读。
- 📱 **原生级 PWA**：深度适配 iOS/Android 安全区域，支持沉浸式全屏阅读，支持离线访问。
- 🚀 **极致性能**：基于流式加载与章节预锁机制，秒开大文件，丝滑翻页。
- 🎲 **发现功能**：打破"选择困难症"，随机从书库中抽取一本小说开启新旅程。
- 🌟 **文件管理**：支持在线标星收藏，支持直接从物理磁盘彻底删除文件，提供一键清库维护工具。
- 📖 **智能分页**：基于 CSS Columns 的精确分页算法，自动适配不同屏幕尺寸，提供精美的翻页体验。
- 🎨 **个性化设置**：支持字体大小、行高、主题、亮度、边距、动画开关等多项阅读配置。
- 🐳 **单容器部署**：开发环境分离，生产环境单容器静态托管，开箱即用。
